...
class shallowDropU(nn.Module):

    def __init__(self, num_class):
        super().__init__()
        
        ################################################
        # sweep DROPOUT_RATE between 0 and 1
        DROPOUT_RATE = 0.05
...
#         self.bn8       = nn.BatchNorm2d(512)
        ...
        # 512, 1024 / 1024, 1024
        self.conv9      = nn.Conv2d(128, 256, kernel_size=3, stride=1, padding=0, dilation=1)
        self.bn9       = nn.BatchNorm2d(256)
        self.conv10     = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=0, dilation=1)
...
#         x21 = F.pad(input=x21, pad=(x5.size(dim=3)-x21.size(dim=3),0,0,x5.size(dim=2)-x21.size(dim=2)), mode='constant')
        x21 = F.pad(input=x15, pad=(x5.size(dim=3)-x15.size(dim=3),0,0,x5.size(dim=2)-x15.size(dim=2)), mode='constant')
